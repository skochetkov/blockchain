...
/**
 * Unspent Transaction Output which contains the hash of the transaction from which it originates
 * as well as its index within that transaction.
 *
 * Wiki:
 * In cryptocurrencies, an unspent transaction output (UTXO) is an output of a blockchain transaction that has not been spent,
 * i.e. used as an input in a new transaction.
 * Outputs are a superset of UTXOs. Accordingly, UTXOs are a subset of the outputs superset.
 * In the case of a valid blockchain transaction, unspent outputs (and only unspent outputs) may be used to effect further transactions.
